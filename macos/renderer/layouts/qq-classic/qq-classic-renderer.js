/* Adapted from zhulin025/Codex-QQ-Skin under MIT; see NOTICE.md. */
((cssText, artDataUrl, petDataUrl, retroFrameDataUrl, qqAvatarDataUrl, themeConfig) => {
  ...
  const COMPONENTS = THEME.layoutComponents && typeof THEME.layoutComponents === "object"
  ...
  const THEME_VARIABLES = [
    ...
    "--dream-skin-user-avatar", "--dream-skin-assistant-avatar",
  ];
  const installToken = {};
  ...
  if (dreamState?.cleanup) {
    try { dreamState.cleanup(); } catch {}
  }
  ...
  const applyTheme = (root, shell) => {
    ...
    setStyleProperty(root, "--dream-skin-user-avatar", avatarDataUrls.user
      ? `url("${avatarDataUrls.user}")` : "none");
    setStyleProperty(root, "--dream-skin-assistant-avatar", avatarDataUrls.assistant
      ? `url("${avatarDataUrls.assistant}")` : "none");
  };
  ...
  const ensureCompanion = () => {
    ...
    if (!companionParts || companionParts.companion !== companion) {
      companionParts = {
        ...
        title: companion.querySelector(".qq-skin-companion-title span"),
        status: companion.querySelector(".qq-skin-pet-status span"),
      };
    }
    if (companionParts.image && companionParts.image.src !== petUrl) companionParts.image.src = petUrl;
    setTextContent(companionParts.title, COMPONENTS.companionTitle || "Codex 伙伴");
    setTextContent(companionParts.status, COMPONENTS.companionStatus || "在线 · 随时待命");
    return companion;
  };
  ...
  const ensureHomePet = (home) => {
    ...
    if (!home || COMPONENTS.homePet === false) {
      pet?.remove();
      return null;
    }
  ...
  const ensureRetroShell = () => {
    ...
    if (COMPONENTS.retroHeader === false && COMPONENTS.toolbar === false) {
      retroShell?.remove();
    ...
    setTextContent(retroShellParts.title, `${COMPONENTS.windowTitle || "Codex 2007"} - ${findRetroTitle()}`);
    return retroShell;
  };
  ...
  const ensureRetroProfile = () => {
    ...
    if (!retroProfileParts || retroProfileParts.profile !== profile) {
      retroProfileParts = {
        ...
        status: profile.querySelector("span"),
      };
    }
    const name = COMPONENTS.profileName || String(profileButton.textContent || "Codex 用户").replace(/\s+/g, " ").trim();
    ...
    const statusTextNode = retroProfileParts.status
      ? [...retroProfileParts.status.childNodes].find((node) => node.nodeType === 3) : null;
    if (statusTextNode) statusTextNode.textContent = ` ${COMPONENTS.profileStatus || "在线"} `;
    return profile;
  };
  ...
  const markChatAvatars = () => {
    ...
    const modern = [...document.querySelectorAll("[data-content-search-unit-key]")];
    const candidates = modern.length ? modern
    ...
    for (const message of document.querySelectorAll(".dream-skin-chat-message")) {
      ...
      message.classList.remove("dream-skin-chat-message", "dream-skin-chat-message-user", "dream-skin-chat-message-assistant");
      [...message.children].filter((child) => child.classList?.contains("dream-skin-chat-avatar"))
  ...
  const applyRootState = (root) => {
    ...
    setAttribute(root, "data-qq-retro-header", COMPONENTS.retroHeader === false ? "off" : "on");
    setAttribute(root, "data-qq-toolbar", COMPONENTS.toolbar === false ? "off" : "on");
  ...
  window[STATE_KEY] = {
    ...
    layoutId: "qq-classic",
    paletteId: THEME.paletteId || "classic-blue",
