...
preferences_source="$STATE_ROOT/.theme-preferences.$$"
cleanup_preferences() { /bin/rm -f "$preferences_source"; }
trap cleanup_preferences EXIT
if [ -f "$THEME_DIR/theme.json" ]; then
  /bin/cp -f "$THEME_DIR/theme.json" "$preferences_source"
  /bin/chmod 600 "$preferences_source"
fi
...
if [ -f "$preferences_source" ]; then
  "$NODE" "$SCRIPT_DIR/update-theme-preferences.mjs" inherit \
    --theme-dir "$THEME_DIR" --from "$preferences_source" >/dev/null
fi
