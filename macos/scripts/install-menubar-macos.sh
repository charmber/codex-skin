...
if [ -d "$INSTALL_ROOT" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  for name in pause-dream-skin-macos.sh status-dream-skin-macos.sh apply-from-menubar-macos.sh \
    switch-theme-macos.sh switch-palette-macos.sh load-image-theme-macos.sh install-menubar-macos.sh \
    configure-reading-panel-macos.sh customize-header-text-macos.sh; do
...
/bin/chmod 755 \
  ...
  "$PROJECT_ROOT/scripts/switch-palette-macos.sh" \
  "$PROJECT_ROOT/scripts/load-image-theme-macos.sh" \
  "$PROJECT_ROOT/scripts/configure-reading-panel-macos.sh" \
  "$PROJECT_ROOT/scripts/customize-header-text-macos.sh" \
