...
if [ "$RESET_DEMO" = "true" ]; then
  ...
  if [ -z "$THEME_NAME" ]; then
    THEME_NAME="$(/usr/bin/osascript -e 'text returned of (display dialog "给这张背景起个名字" default answer "我的背景" buttons {"取消", "继续"} default button "继续")')" \
  ...
  theme_args=(custom --output-dir "$THEME_DIR" --image "$image_name" --background-name "$THEME_NAME")
  if [ -f "$THEME_DIR/theme.json" ]; then theme_args+=(--inherit-theme "$THEME_DIR/theme.json"); fi
  [ -n "$TAGLINE" ] && theme_args+=(--tagline "$TAGLINE")
  [ -n "$QUOTE" ] && theme_args+=(--quote "$QUOTE")
